...
with open(InputFile, 'r', encoding='utf8') as file_in:
    ...
    for row in csvreader:
        ...
        if FileNumber % 100000 == 0:
            FolderNumber = str(FileNumber)
            if not os.path.exists(OutputDir + FolderNumber):
                os.makedirs(OutputDir + FolderNumber)
        FileNumber += 1
        

        Filename_Clean = row[Filename_Var] + ';' + str(FileNumber)
        print("Writing file: " + Filename_Clean)
        #depending on your input file, you might need to add an encoding
        #such as 'utf-8' after the 'rb'
        with open(OutputDir + str(FolderNumber) + '/' + Filename_Clean + ".txt", 'w', encoding='utf8') as OutputFile:
